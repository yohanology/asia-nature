import { Navbar } from '../components/Navbar';
import { Hero } from '../components/Hero';
import { Features } from '../components/Features';
import { Testimonials } from '../components/Testimonials';
import { Founder } from '../components/Founder';
import { Footer } from '../components/Footer';
import { OfficeVideo } from '../components/OfficeVideo';
import { Button } from '../components/ui/Button';
import { useLanguage } from '../contexts/LanguageContext';
import { content } from '../translations/content';

export function Home() {
  const { language } = useLanguage();
  const t = content[language];
  const about = t.about;
  const cta = t.cta;

  return <div className="min-h-screen bg-[#F9F8F6] text-[#1A1A1A] font-sans selection:bg-[#2F4F3E] selection:text-white">
    <Navbar />
    <main>
      <Hero />

      <section id="societe" className="py-24 bg-[#E8E4D9]/30">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="relative">
              <div className="absolute -top-4 -left-4 w-24 h-24 bg-[#D4C5B0] -z-10 rounded-full opacity-50"></div>
              <img src="/asia_nature_bureau_shenzhen_1.webp" alt={t.alts.officeMeeting} loading="lazy" decoding="async" className="w-full h-auto shadow-xl rounded-sm grayscale hover:grayscale-0 transition-all duration-700" />
              <div className="absolute -bottom-6 -right-6 bg-white p-6 shadow-lg max-w-xs border-l-4 border-[#2F4F3E]">
                <p className="font-serif italic text-[#2F4F3E] text-lg">
                  {about.quote}
                </p>
              </div>
            </div>
            <div>
              <span className="text-[#2F4F3E] font-medium tracking-widest uppercase text-xs mb-4 block">
                {about.badge}
              </span>
              <h2 className="text-4xl font-serif font-bold text-[#2F4F3E] mb-6">
                {about.title}
              </h2>
              <p className="text-lg text-[#4A4A4A] leading-relaxed mb-6">
                <span className="font-semibold text-[#2F4F3E]">Asia Nature Limited</span>{' '}
                {about.p1}{' '}
                <span className="font-semibold">{about.p1Bold1}</span>{' '}
                {about.p1Mid}{' '}
                <span className="font-semibold">{about.p1Bold2}</span>
                {about.p1End}{' '}
                <span className="font-semibold">{about.p1Bold3}</span>
                {about.p1End2}
              </p>
              <p className="text-[#4A4A4A] leading-relaxed mb-6">
                {about.p2Start}{' '}
                <span className="font-semibold">{about.p2Bold1}</span>
                {about.p2Mid}{' '}
                <span className="font-semibold">{about.p2Bold2}</span>
                {about.p2End}
              </p>
              <p className="text-[#4A4A4A] leading-relaxed mb-8">
                {about.p3}{' '}
                <span className="font-semibold text-[#2F4F3E]">{about.p3Bold}</span>{' '}
                {about.p3End}
              </p>
              <Button variant="outline" withDownArrow onClick={() => {
                document.getElementById('bureaux')?.scrollIntoView({
                  behavior: 'smooth',
                  block: 'start'
                });
              }}>
                {about.ctaOffice}
              </Button>
            </div>
          </div>
        </div>
      </section>

      <OfficeVideo />

      <Features />

      <Testimonials />

      <Founder />

      <section className="py-24 md:py-32 bg-white text-center border-t border-[#E8E4D9]">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#2F4F3E] mb-6">
            {cta.title}
          </h2>
          <p className="text-xl text-[#4A4A4A] mb-10 max-w-2xl mx-auto font-light">
            {cta.description}
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Button size="lg" withArrow asChild>
              <a href="#contact">{cta.ctaPrimary}</a>
            </Button>
            <Button variant="outline" size="lg" asChild>
              <a href="#contact">{cta.ctaSecondary}</a>
            </Button>
          </div>
        </div>
      </section>
    </main>
    <Footer />
  </div>;
}
